...
describe('GithubService', () => {
  ...
  it('should getRepositories', inject([GithubService, HttpTestingController], (githubService: GithubService, http: HttpTestingController) => {
    const response: any = mockResponse;
    const pageNumber = 1;
    githubService.query = { query: 'Angular10'};

    githubService.searchRepositories(1).subscribe((res) => {
      // expect(response).toEqual(res); // Types don't match
    });
    const req = http.expectOne(`${url}/search/repositories?q=${githubService.query.query}&page=${pageNumber}`, 'Expect request to get repositories based on the query');
    expect(req.request.url).toEqual(`${url}/search/repositories?q=${githubService.query.query}&page=${pageNumber}`);
    expect(req.request.method).toEqual('GET');
    req.flush((response));
    http.verify();
  }));

  it('shoud get a single repository', inject([GithubService, HttpTestingController], (githubService: GithubService, http: HttpTestingController) => {
    const githubResponse: any = {
      "id": 24195339,
      ...
      "updated_at": "2020-10-24T11:04:20Z",
      "pushed_at": "2020-10-24T09:17:17Z",
      ...
      "size": 265085,
      "stargazers_count": 67034,
      "watchers_count": 67034,
      "language": "TypeScript",
      "has_issues": true,
      "has_projects": true,
      "has_downloads": true,
      ...
      "network_count": 17785,
      "subscribers_count": 3220
    };
    // Karma zegt dat de methode 'getRepositoryById' niet bestaat
    // const getRepositorySpy = spyOn(githubService, 'getRepositoryById').and.returnValue(githubResponse);

    githubService.getRepositoryById(githubResponse.id).subscribe(data => {
      expect(githubResponse).toEqual(data);
    });
